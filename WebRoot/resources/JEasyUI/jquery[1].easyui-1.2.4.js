...
(function($) {
	...
	var view = $.extend({}, $.fn.datagrid.defaults.view, {
		...
		onAfterRender : function(target) {
			...
			//是否关闭分类组
			if(opts.isSortClosed){
				$('<td style="border:0"><div class="datagrid-row-expander datagrid-row-expand" style="width:25px;height:16px;cursor:pointer"></div></td>')
					.insertBefore(gridGroup.find('td'));
				var gridGroup11 = gridView.find('div.datagrid-group');
				gridGroup11.next('table').hide();
			}else {
